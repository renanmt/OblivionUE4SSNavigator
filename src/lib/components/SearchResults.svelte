<script lang="ts">
  ...
  function getTypeDisplay(typeInfo: any): string {
    ...
    if (typeof typeInfo.type === 'string') {
      ...
      if (typeInfo.isUnion && typeInfo.subTypes)
        return typeInfo.subTypes.join(' | ');
      ...
      if (referencedEntity) {
        ...
        if (typeInfo.isMap && typeInfo.subTypes && typeInfo.subTypes.length >= 2) {
          const keyType = typeof typeInfo.subTypes[0] === 'number' 
            ? (database.entityMap.get(typeInfo.subTypes[0])?.name || 'unknown')
            : typeInfo.subTypes[0];
          const valueType = typeof typeInfo.subTypes[1] === 'number' 
...
{:else}
  <div class="my-6">
    <!-- Tabs -->
    <div class="border-b border-[#15192b] mb-4">
      <ul class="flex -mb-px">
        {#if entities.length > 0}
          <li class="mr-1">
            <button 
              class="inline-block py-2 px-4 font-medium text-center {activeTab === 'entities' ? 'border-b-2 border-[#3a4577] text-[#3a4577]' : 'text-gray-400 hover:text-gray-300'}" 
        ...
        {#if properties.length > 0}
          <li class="mr-1">
            <button 
              class="inline-block py-2 px-4 font-medium text-center {activeTab === 'properties' ? 'border-b-2 border-[#3a4577] text-[#3a4577]' : 'text-gray-400 hover:text-gray-300'}" 
        ...
        {#if functions.length > 0}
          <li class="mr-1">
            <button 
              class="inline-block py-2 px-4 font-medium text-center {activeTab === 'functions' ? 'border-b-2 border-[#3a4577] text-[#3a4577]' : 'text-gray-400 hover:text-gray-300'}" 
        ...
        {#if params.length > 0}
          <li class="mr-1">
            <button 
              class="inline-block py-2 px-4 font-medium text-center {activeTab === 'params' ? 'border-b-2 border-[#3a4577] text-[#3a4577]' : 'text-gray-400 hover:text-gray-300'}" 
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {#each entities as entity}
            <a 
              ...
              class="p-3 border border-[#15192b] rounded-lg hover:shadow-md transition-shadow bg-[#0f121e] hover:bg-[#15192b]"
              title={entity.name}
            >
              <h3 class="font-medium overflow-hidden text-ellipsis text-gray-100">{truncateName(entity.name)}</h3>
      ...
      {#if activeTab === 'properties' && properties.length > 0}
        <div class="overflow-x-auto">
          <table class="w-full">
            <thead>
              <tr class="bg-[#111422]">
                <th class="text-left p-2 text-gray-300">Name</th>
                <th class="text-left p-2 text-gray-300">Type</th>
                <th class="text-left p-2 text-gray-300">Parent</th>
              </tr>
            </thead>
            <tbody>
              {#each properties as property}
                <tr class="border-b border-[#15192b] hover:bg-[#15192b]">
                  ...
                  <td class="p-2">
                    <a href={getEntityLink(property.parent)} class="text-[#3a4577] hover:text-[#4a559a] hover:underline" title={getEntityName(property.parent)}>
      ...
      {#if activeTab === 'functions' && functions.length > 0}
        <div class="space-y-3">
          {#each functions as func}
            <div class="p-3 border border-[#15192b] rounded-lg bg-[#0f121e]">
              <div class="flex justify-between items-start">
                <div>
                  ...
                  <p class="text-sm text-gray-400">
                    Returns: {func.return ? 
                      getTypeDisplay(func.return) : 'void'}
                  </p>
                </div>
                <a href={getEntityLink(func.parent)} class="text-sm text-[#3a4577] hover:text-[#4a559a] hover:underline">
          ...
          <table class="w-full">
            <thead>
              <tr class="bg-[#111422]">
                <th class="text-left p-2 text-gray-300">Name</th>
                <th class="text-left p-2 text-gray-300">Type</th>
                <th class="text-left p-2 text-gray-300">Function</th>
              </tr>
            </thead>
            <tbody>
              {#each params as param}
                <tr class="border-b border-[#15192b] hover:bg-[#15192b]">
                  ...
                  <td class="p-2">
                    {#if param.parent}
                      <a href={getEntityLink(param.parent)} class="text-[#3a4577] hover:text-[#4a559a] hover:underline">
